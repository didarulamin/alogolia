...
const dummy = [
  ...
  },
  {
    ...
    _highlightResult: {
      ...
      url: {
        value: "https://pipedream.com/blog/reindex-algolia-on-github-releases/",
        matchLevel: "none",
        matchedWords: [],
      },
      author: {
  ...
  },
  {
    ...
    title: "Solar energy can now be stored for up to 18 years, say scientists",
  ...
  },
  {
    ...
    title: "Seal taken into care after being fed 'sandwiches and doughnuts'",
  ...
  },
  {
    ...
    _tags: ["story", "author_phendrenad2", "story_31041331", "ask_hn"],
    objectID: "31041331",
    _highlightResult: {
      title: {
...
test("gets raw json data", async () => {
  render(
    <MemoryRouter initialEntries={[`/post/${postId}`]}>
      <Routes>
        <Route path="/post/:postId" element={<JSON rows={dummy} />}></Route>
      </Routes>
    </MemoryRouter>
  );
  expect(await screen.findByText("row json page")).toBeInTheDocument();
  const json = screen.getByTestId("json-data");
  //   console.log(json.innerHTML);
  //   expect(json.innerHTML).toBeInTheDocument();
  expect(json.innerHTML).toContain(
    "Elon Musk Also Threatened to Buy My Company. Here’s How We Handled It"
  );
});
